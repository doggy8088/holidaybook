...
Resolve whether a Taiwan date is a holiday or workday using the official `holidaytw` CLI when possible, automatically adapting to whatever tools are available in the current environment, and falling back to the static JSON API only when no CLI install path works.
...
Attempt each candidate that is applicable to the current environment, in order. If one fails (command not found, install error, network failure for that step), do not stop or give up — continue to the next candidate. Only after every applicable candidate below, including the static API fallback in step 3, has failed should you report retrieval failure.

1. **Already installed** — check `command -v holidaytw` (POSIX shells) or `Get-Command holidaytw` (PowerShell). If found, use it directly.
2. **macOS/Linux** — install via the official one-liner, then re-resolve the path (step 4):

   ```sh
   curl -fsSL https://raw.githubusercontent.com/doggy8088/holidaybook/master/install.sh | sh
   ```
3. **Windows PowerShell** — install via the official one-liner, then re-resolve the path (step 4):

   ```powershell
   irm https://raw.githubusercontent.com/doggy8088/holidaybook/master/install.ps1 | iex
   ```
4. **Resolve the installed path** — after install.sh/install.ps1 finishes, `holidaytw` is not guaranteed to be on `PATH` yet in the current shell. Re-check `PATH` first; if still not found, use the known default install locations: `~/.local/bin/holidaytw` (or the directory passed via `--dir`) on macOS/Linux, `%LOCALAPPDATA%\Programs\holidaytw\holidaytw.exe` (or the `-InstallDir` used) on Windows. Invoke the resolved full path directly instead of re-running the installer.
5. **All install/CLI candidates failed or are unsupported here** (no network access to GitHub, restricted or sandboxed shell, unsupported OS/architecture, permission errors) — use the HTTPS static JSON API fallback (step 3 below) instead of retrying a failed install.

A failed candidate never by itself means the query failed — it only means try the next candidate, which may still return valid data (including the API fallback). Use only official sources: `github.com/doggy8088/holidaybook`, `raw.githubusercontent.com/doggy8088/holidaybook`, and `https://holiday.gh.miniasp.com`. Never use a third-party mirror or proxy for install or data, and never disable TLS verification.

Do not execute `npx holidaytw` or `npm install holidaytw` until this skill is updated after the official npm package has been published and its ownership is established. Before that bootstrap publication, the unclaimed registry name is not a trusted installation source.
